...
export default function Learn() {
  ...
  // FIX: lưu case riêng cho từng step-substep
  const [activeCaseIndexes, setActiveCaseIndexes] = useState<
  ...
  const subSwiperRefs = useRef<Record<number, any>>({});
  ...
  const currentCaseKey = `${activeStepIdx}-${activeSubStepIdx}`;

  const currentCaseIdx = activeCaseIndexes[currentCaseKey] || 0;
    ...
    return text.split("\n").map((line, i) => (
      <span key={i}>
        {line.split("**").map((part, j) =>
          j % 2 === 1 ? (
            <strong key={j} className="text-slate-900 font-black">
              {part}
            </strong>
          ) : (
            part
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-[calc(100vh-64px)] flex flex-col relative overflow-hidden bg-[#FFFBF0]"
    >
      {/* Background */}
      <div
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 ml-6">
            <button
              onClick={handlePrev}
              disabled={
                activeStepIdx === 0 && activeSubStepIdx === 0
              }
              className={`p-2.5 rounded-xl transition-all ${
                activeStepIdx === 0 &&
                activeSubStepIdx === 0
                  ? "bg-slate-100 text-slate-300"
                  : "bg-white text-slate-700 shadow-md hover:bg-slate-50 active:scale-95"
              }`}
            >
              <ChevronLeft size={20} />
            </button>
          ...
          >
            {STEPS.map((step, stepIdx) => (
              <SwiperSlide
                key={stepIdx}
                className={
                  stepIdx === activeStepIdx ? "z-10" : "z-0"
                }
              >
                <div
                  className={`w-full ${
                ...
                >
                  ...
                  <Swiper
                    ...
                    watchSlidesProgress
                    onSwiper={(swiper) => {
                      subSwiperRefs.current[stepIdx] = swiper;
                    }}
                    onSlideChange={(swiper) => {
              ...
              {hasAnimation && (
                <div className="flex gap-3 pointer-events-auto">
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center bg-white text-slate-900 rounded-2xl shadow-xl hover:scale-110 active:scale-90 transition-all"
                  >
                    {isPaused ? (
                      <Play
                        size={20}
